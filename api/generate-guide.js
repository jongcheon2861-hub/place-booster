...
module.exports = async (req, res) => {
  ...
  const { industry, region, placeName, speciality, topKeywords } = req.body || {};
  ...
  const TEMPLATES = {
    restaurant: {
      ...
      keywordTips: [
        '소개글 첫 문장에 "지역+업종" 키워드를 넣으세요 (예: 홍대 맛집).',
        '메뉴 설명에 대표 키워드를 자연스럽게 1회 삽입하세요.',
        '"오시는 길"에 지역 키워드를 한 번 더 반복하세요.',
        '계절 메뉴가 있다면 시즌 키워드(여름 보양식, 겨울 국물요리 등)를 활용하세요.'
      ]
    },
    cafe: {
      name: '카페',
      ...
      descSections: [
        { title: '시그니처 메뉴', hint: '대표 음료·디저트 3~5개를 작성하세요.' },
        { title: '공간 소개', hint: '좌석 수, 콘센트 여부, 단체석, 테라스 등을 작성하세요.' },
        { title: '오시는 길', hint: '가까운 역·정류장, 주차 안내를 작성하세요.' },
    ...
    beauty: {
      name: '미용실/뷰티',
      intro: [
        `${region} 미용실 ${placeName}입니다. ${speciality || '헤어 컬러·커트 전문'}으로 고객 맞춤 스타일을 제안합니다.`,
        `${placeName} — ${region}에서 ${speciality || '트렌디한 헤어 디자인'}을 만나보세요. 1:1 상담으로 최적의 스타일을 찾아드립니다.`,
      ...
      descSections: [
        ...
        { title: '예약 안내', hint: '예약 방법(네이버 예약, 전화), 영업시간, 휴무일을 작성하세요.' }
      ],
      keywordTips: [
        '"지역+미용실" 또는 "지역+헤어숍"을 첫 문장에 넣으세요.',
        '시술명(발레아쥬, 볼륨매직 등)을 구체적으로 키워드화하세요.',
        '"남자 미용실", "여성 전문" 등 타깃 키워드를 활용하세요.',
        '시즌별(졸업식 헤어, 여름 단발 등) 키워드를 소개글에 반영하세요.'
      ]
    },
    hospital: {
      name: '병원/의원',
      intro: [
        `${region} ${speciality || '정형외과/피부과'} ${placeName}입니다. 정확한 진단과 맞춤 치료로 건강을 지켜드립니다.`,
        `${placeName} — ${region}에서 ${speciality || '비수술 치료와 체계적 재활'}을 전문으로 하는 의원입니다.`,
        `${region} ${speciality || '전문의 진료'}를 찾으신다면 ${placeName}. 풍부한 임상 경험으로 최선의 결과를 약속합니다.`
      ],
      descSections: [
      ...
      keywordTips: [
        '"지역+진료과목" 키워드를 첫 문장에 배치하세요 (예: 강남 정형외과).',
        '구체적 치료명(도수치료, 체외충격파 등)을 키워드로 활용하세요.',
        '"야간진료", "주말진료" 등 편의 키워드를 넣으면 검색 유입이 늘어납니다.',
    ...
    fitness: {
      ...
      keywordTips: [
        ...
        '"다이어트"보다 "지역+피트니스" 키워드가 플레이스 노출에 유리합니다.',
        '"여성전용", "24시" 등 차별화 키워드를 활용하세요.',
        '트레이너 자격증(NSCA 등)을 키워드로 활용하면 신뢰도가 올라갑니다.'
      ]
    },
    education: {
      name: '학원/교육',
      intro: [
        `${region} ${speciality || '영어/수학 전문'} 학원 ${placeName}입니다. 맞춤 커리큘럼으로 성적 향상을 이끕니다.`,
        `${placeName} — ${region}에서 ${speciality || '소수 정예 수업'}으로 차별화된 교육을 제공합니다.`,
        `${region} 학원 ${placeName}. ${speciality || '입시·내신 전문 강사진'}이 함께합니다.`
      ],
      descSections: [
        { title: '교육 과정', hint: '과목, 대상(초등/중등/고등), 반 구성을 작성하세요.' },
      ...
      keywordTips: [
        ...
        '학년별 키워드(초등 영어, 고등 수학 등)가 검색량이 더 높을 수 있습니다.'
      ]
    },
    pet: {
      name: '반려동물',
      intro: [
        `${region} ${speciality || '애견미용·호텔'} ${placeName}입니다. 반려동물의 건강과 행복을 최우선으로 생각합니다.`,
        `${placeName} — ${region}에서 ${speciality || '소형견·대형견 전문 케어'}를 제공합니다.`,
      ...
      descSections: [
        ...
        { title: '오시는 길', hint: '역·정류장, 주차 안내를 작성하세요.' },
        { title: '이용 안내', hint: '영업시간, 예약 방법, 필요 서류(접종증명 등)를 작성하세요.' }
      ],
      keywordTips: [
    ...
    service: {
      name: '서비스/기타',
      intro: [
        `${region} ${speciality || '전문 서비스'} ${placeName}입니다. 고객 만족을 최우선으로 합니다.`,
  ...
  /* ──────────────────────────────────────
     2. 템플릿 선택 & 소개글 생성
  ────────────────────────────────────── */
  const template = TEMPLATES[industry] || TEMPLATES['service'];
  const keywords = topKeywords || [];

  // 소개글 후보 3개 생성
  const introOptions = template.intro.map((text, i) => {
    return { version: i + 1, text };
  });

  /* ──────────────────────────────────────
     3. 키워드 삽입 체크
  ────────────────────────────────────── */
  const keywordCheck = keywords.map(kw => {
    const allTexts = template.intro.join(' ');
    const found = allTexts.includes(kw);
    return {
      keyword: kw,
      includedInIntro: found,
      suggestion: found
        ? '✅ 소개글에 포함됨'
        : `⚠️ 소개글에 "${kw}" 미포함 → "오시는 길" 또는 "메뉴 설명"에 자연스럽게 삽입하세요`
    };
  });
  ...
  /* ──────────────────────────────────────
     4. 상세 설명 섹션별 작성 가이드
  ────────────────────────────────────── */
  const descriptionGuide = template.descSections.map(section => {
    // 해당 섹션에 삽입하면 좋을 키워드 제안
    const suggestedKw = keywords.length > 0
  ...
  /* ──────────────────────────────────────
     5. 키워드 최적화 팁 (업종별)
  ────────────────────────────────────── */
  const optimizationTips = template.keywordTips;
  ...
  const keywordPlacementMap = {
    ...
    '사진 설명(ALT)': '사진 업로드 시 파일명에 키워드 포함 권장'
  };
  ...
  res.status(200).json({
    ...
    warnings: [
      ...
      '대형 키워드(맛집, 카페 등)는 초반에 피하고, 소형→중형→대형 순서로 공략하세요.'
    ]
  });
};
